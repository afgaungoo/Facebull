...
struct arc** initialise_source_array(int no_of_nodes){ // Will be initialised full of garbage (not now I'm using calloc)
	struct arc** source_array=(arc**)calloc(no_of_nodes,sizeof(struct arc));
...
void arrange_list(struct arc* lookup_array, struct arc** source_array){
int position=0;	
printf("%s", "hellos");
for (int i=0; i<no_of_lines, i++;){
	printf("%s", "hello2");
	if ((lookup_array[i]).source==2){
		printf("%s", "hello3");
		//printf("%d %s", i,"\n");
