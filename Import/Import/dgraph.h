...
typedef struct dgraph_edge {
    int vertex_no;			// target?
    int dist;				// cost
    struct dgraph_edge *next;	//pointer to next edge
} dgraph_edge_t;

/* Vertex structure type.
 */
typedef struct dgraph_vertex {
    dgraph_edge_t *first_edge;	//pointer to first edge
    dgraph_edge_t *last_edge;	//pointer to last edge
} dgraph_vertex_t;
...
typedef struct dgraph {
    int n;			//nodeCount
    dgraph_vertex_t *vertices; //pointer to vertices
} dgraph_t;
